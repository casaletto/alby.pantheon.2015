...
namespace alby.pantheon.codegen.storedProcedure
{
	...
	{
		public int SolicitorGet
		(
			sds.SqlConnection connˡ, 
			int? PartyID, 
			string UpdateUserCode, 
			DateTime? UpdateDateTime, 
			out scg.List<SolicitorGet٠rs1> rsˡ1, 
			out scg.List<SolicitorGet٠rs2> rsˡ2, 
			out scg.List<SolicitorGet٠rs3> rsˡ3, 
			sds.SqlTransaction tranˡ = null
		)
		{
			const string schemaˡ = "dbo" ; 
			const string spˡ = "SolicitorGet" ; 
			...
			rsˡ1 = base.ToRecordsetˡ<SolicitorGet٠rs1>( dsˡ, 1 ) ;
			rsˡ2 = base.ToRecordsetˡ<SolicitorGet٠rs2>( dsˡ, 2 ) ;
			rsˡ3 = base.ToRecordsetˡ<SolicitorGet٠rs3>( dsˡ, 3 ) ;
