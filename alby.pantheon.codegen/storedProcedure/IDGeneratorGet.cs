...
namespace alby.pantheon.codegen.storedProcedure
{
	...
	{
		public int IDGeneratorGet
		(
			sds.SqlConnection connˡ, 
			string AssignedType, 
			string UpdateUserCode, 
			DateTime? UpdateDateTime, 
			out scg.List<IDGeneratorGet٠rs1> rsˡ1, 
			sds.SqlTransaction tranˡ = null
		)
		{
			const string schemaˡ = "dbo" ; 
			const string spˡ = "IDGeneratorGet" ; 
			 
			scg.List<sds.SqlParameter> parametersˡ = new scg.List<sds.SqlParameter>() ;
			sds.SqlParameter paramˡAssignedType = base.AddParameterˡ( parametersˡ, "@AssignedType", AssignedType, sd.SqlDbType.NVarChar, true, 100, null, null ) ; 
			...
			rsˡ1 = base.ToRecordsetˡ<IDGeneratorGet٠rs1>( dsˡ, 1 ) ;
