...
namespace alby.pantheon.codegen.storedProcedure
{
	...
	{
		public int LeaseByStatus
		(
			sds.SqlConnection connˡ, 
			string status, 
			string UpdateUserCode, 
			DateTime? UpdateDateTime, 
			out scg.List<LeaseByStatus٠rs1> rsˡ1, 
			sds.SqlTransaction tranˡ = null
		)
		{
			const string schemaˡ = "dbo" ; 
			const string spˡ = "LeaseByStatus" ; 
			 
			scg.List<sds.SqlParameter> parametersˡ = new scg.List<sds.SqlParameter>() ;
			sds.SqlParameter paramˡstatus = base.AddParameterˡ( parametersˡ, "@status", status, sd.SqlDbType.NChar, true, 100, null, null ) ; 
			...
			rsˡ1 = base.ToRecordsetˡ<LeaseByStatus٠rs1>( dsˡ, 1 ) ;
