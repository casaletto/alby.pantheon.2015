...
namespace alby.pantheon.codegen.storedProcedure
{
	...
	{
		public int FranchiseGet
		(
			sds.SqlConnection connˡ, 
			int? PartyID, 
			string UpdateUserCode, 
			DateTime? UpdateDateTime, 
			out scg.List<FranchiseGet٠rs1> rsˡ1, 
			out scg.List<FranchiseGet٠rs2> rsˡ2, 
			out scg.List<FranchiseGet٠rs3> rsˡ3, 
			sds.SqlTransaction tranˡ = null
		)
		{
			const string schemaˡ = "dbo" ; 
			const string spˡ = "FranchiseGet" ; 
			...
			rsˡ1 = base.ToRecordsetˡ<FranchiseGet٠rs1>( dsˡ, 1 ) ;
			rsˡ2 = base.ToRecordsetˡ<FranchiseGet٠rs2>( dsˡ, 2 ) ;
			rsˡ3 = base.ToRecordsetˡ<FranchiseGet٠rs3>( dsˡ, 3 ) ;
